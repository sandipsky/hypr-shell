...
src/services/idle.{hpp,cpp}             idle daemon: ext-idle-notify-v1 stages (screen off /
                                        lock / suspend) with fade grace period; protocol glue is
                                        generated by wayland-scanner in meson.build
...
- [ ] **Phase 5 — Lock & idle**: lock screen via ext-session-lock
      ...
      *(pulled forward 2026-09-03: the idle service landed — Noctalia's
      ...
      compositor honors inhibitors for v1 notifications. "Lock" is
      `loginctl lock-session` until the lock screen exists. Settings expose
      only the three timeouts, per user)*
