...
| `~/.config/hypr-shell/presets/<name>.json` | Presets from the settings app's Presets page: `{"name", "saved", "config": {…}}`, a snapshot of the whole config.json per file. Switching writes that snapshot back as config.json. |
...
| `HS_SETTINGS_PAGE=<tag>` | Settings app: jump to a Bar module subpage (`workspaces`, `clock`, `active_window`, `battery`, `bluetooth`, `notifications`, ...) or a sidebar page (`wallpaper_page`, `night_light_page`, `launcher_page`, `session_page`, `lock_page`, `users_page`, `idle_page`, `osd_page`, `notifications_page`, `about_page`) |
| `HS_USERS_ADD=1` / `HS_USERS_PASSWORD=1` / `HS_USERS_PICTURE=<image>` | Settings app, Users page: open the Add User / Change Password dialog, or apply a picture to the current account (writes `~/.face`; `HS_USERS_NO_PKEXEC=1` skips the SDDM copy so no prompt appears) |
| `HS_UI_FONT_SIZE=<pt>` | Settings app, User interface page: set the Font size row after 1.5 s — this writes the **system** font (gsettings `font-name` + `gtk-{3,4}.0/settings.ini` + `fontconfig/conf.d/50-hypr-shell-sans-serif.conf`); back them up and restore |
| `HS_UI_CURSOR=<theme>:<size>` | Settings app, User interface page: set the Cursor rows after 1.5 s and run the change handler (config.json, gsettings, settings.ini, `~/.icons/default`, SDDM). Add `HS_CURSOR_SDDM_ROOT=<dir>` to write the SDDM drop-in under that directory instead of raising a pkexec prompt |
| `HS_UI_ICON_THEME=<id>` | Settings app, User interface page: select that icon theme after 1.5 s and run the handler (gsettings + settings.ini) |
...
| `HS_SETTINGS_TIMING=1` | Settings app: print milliseconds since `main()` at each startup phase (build, populate, present, realize, first frames) and at every config.json write |
